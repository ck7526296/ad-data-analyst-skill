...
- 调用 `search_ad_plans({ "planName": "5月回流-安卓", "responseMode": "summary" })`。
...
- 调用 `search_ad_plans({ "manager": "张三", "returnDataCode": 2611, "queryDate": "YYYY-MM-DD", "responseMode": "summary" })`。
...
- 定位计划后调用 `get_upload_status`，必要时补 `get_plan_trend(responseMode="summary")`；只有设备、回传或留存也需要核对时再补 `get_quality_diagnosis_context(responseMode="summary")`。
...
- 回答应要求重新确认计划或改做 `search_ad_plans({ "returnDataCode": 2611, "responseMode": "summary" })` 查关联计划。
...
- 先定位计划；如果有 `planId`，调用 `get_return_data_raw_context({ "planId": ..., "returnDataCode": 2611, "queryDate": "YYYY-MM-DD", "pageSize": 5, "responseMode": "summary" })`。
- 只有用户明确要求扩大样本或完整明细时，才传 `maxSummaryRows`，最大 1000。
...
- 定位计划和“昨天”的明确日期。
- 调用 `get_plan_action_review_context({ "planId": ..., "queryDate": "YYYY-MM-DD", "days": 7, "actionDate": "YYYY-MM-DD", "actionType": "换包", "responseMode": "summary" })`；若只知道大致时间，可省略 `actionDate` 让工具自动取最近操作/变更日期。
...
- 调用 `get_creative_diagnosis_context({ "planId": ..., "queryDate": "YYYY-MM-DD", "topN": 5, "responseMode": "summary" })`。
...
- 调用 `get_external_crowd_pack_plan_comparison({ "externalCrowdPack": "A", "queryDate": "YYYY-MM-DD", "responseMode": "summary" })`。
